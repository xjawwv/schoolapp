    ...
    <div class="flex-1 flex flex-col overflow-y-auto">
      ...
      <main class="p-6 sm:p-8 pt-8 sm:pt-10 max-w-7xl w-full mx-auto space-y-6 sm:space-y-8">
        <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <NuxtLink to="/user/create" class="button-primary flex items-center space-x-2 self-start md:self-auto py-3">
            ...
            <table class="w-full text-left border-collapse">
              <thead>
                <tr class="border-b border-[color:var(--color-border)]">
                  ...
                  <th class="py-3 px-4 text-xs uppercase tracking-widest text-[color:var(--color-muted)] font-bold">NIP / NISN</th>
              ...
              <tbody>
                ...
                >
                  ...
                  <td class="py-3.5 px-4 text-sm font-mono text-[color:var(--color-accent)]">
                    <span v-if="u.role === 'guru'">{{ u.nip || '-' }}</span>
                    <span v-else-if="u.role === 'siswa' || u.role === 'siswi'">{{ u.student?.nisn || '-' }}</span>
        ...
        <div class="w-full max-w-lg bg-[color:var(--color-surface)] border border-[color:var(--color-border)] border-l-4 border-l-[color:var(--color-accent)] p-8 shadow-[--shadow-lg] relative">
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <div class="space-y-1.5">
              ...
              <input v-model="formData.password" type="password" class="input w-full font-mono" placeholder="Minimal 6 karakter" />
            </div>
...
async function handleSubmit() {
  ...
  try {
    let payload: any = {
      ...
      student_id: (formData.value.role === 'siswa' || formData.value.role === 'siswi') ? (formData.value.student_id ? formData.value.student_id : null) : null,
